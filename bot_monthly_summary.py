...
def main():
    ...
    datafilePath = '{}/data_monthly.json'.format(os.getcwd())
    prevData = load_data(datafilePath)
    save_data(datafilePath, data)
    diff = get_diff(data, prevData)
    message = get_message(diff, config['address'], config['tweet_settings']['base_text_monthly'], config['tweet_settings']['include_defiscan_link'])
